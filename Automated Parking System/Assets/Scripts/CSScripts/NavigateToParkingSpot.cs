...
public class NavigateToParkingSpot : Agent
{
    ...
    public override void OnEpisodeBegin()
    {
        ...
        transform.rotation = Quaternion.Euler(0, Random.Range(-90.0f, 90.0f), 0);
    ...
    private void CalculateReward(float rotation, float[] distances, int[] sensorDistances)
    {
        // Add penalty for being to far to spot
        AddReward(distances.Sum() * -0.1f);

        // Add penalty for being to close to cars
        AddReward(sensorDistances.Sum() * -0.5f);


        // Add penalty for wrong rotation
        AddReward(rotation * -0.1f);
    }
}
